...
func numaHasOnlineBusinessPods(machineState state.NUMANodeMap, stateOK bool, numaID int) bool {
	...
	return machineState[numaID].HasOnlineBusinessPods()
}
...
func doNumaMemCompact(metaServer *metaserver.MetaServer, emitter metrics.MetricEmitter, interval time.Duration) {
	...
	for _, numaID := range metaServer.CPUDetails.NUMANodes().ToSliceNoSortInt() {
		// Step 0, a NUMA node hosting online business pods is dirtied: drop its compaction record so
		...
		// Step 0.1, the NUMA node is idle now. If it was already compacted while idle, only
		...
		if last, ok := getNumaLastCompact(numaID); ok {
			if interval <= 0 || nowFn().Sub(last) < interval {
				general.Infof("skip NUMA %d: compacted while idle at %v, interval=%v not elapsed", numaID, last, interval)
				continue
			}
		}

		// Step 1, compact this NUMA node unless its own kcompactd is busy (R/D state). Emit the
...
func NumaMemCompact(conf *coreconfig.Configuration,
	...
	general.Infof("NumaMemCompact: EnableNumaMemCompact enabled")
