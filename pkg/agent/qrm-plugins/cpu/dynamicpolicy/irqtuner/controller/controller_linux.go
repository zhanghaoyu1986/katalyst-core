		...
		if err != nil {
			...
			if len(affinityCPUList) != 1 {
				...
				if affinityCPUList[0] != targetCore {
					general.Errorf("%s nic %s irq %d affinity incorrect cpu %d", IrqTuningLogPrefix, nic, irq, affinityCPUList[0])
					...
					affinityCPUList, err := general.ParseLinuxListFormatFromFile(irqSmpAffinityFile)
					if err != nil {
						general.Errorf("%s after sleep 5s, nic %s failed to ParseLinuxListFormatFromFile(%s), err %s", IrqTuningLogPrefix, nic, irqSmpAffinityFile, err)
					} else {
						general.Infof("%s after sleep 5s, nic %s irq %d affinity cpus (%+v)", IrqTuningLogPrefix, nic, irq, affinityCPUList)
					}
				} else {
