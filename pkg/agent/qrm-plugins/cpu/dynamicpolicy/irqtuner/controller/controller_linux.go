...
func (ic *IrqTuningController) syncNics() ([]*machine.NicBasicInfo, bool, error) {
	...
	if !nicsChanged {
		...
		return []*machine.NicBasicInfo{}, false, nil
	}
...
func (ic *IrqTuningController) adaptIrqAffinityPolicy(oldIndicatorsStats *IndicatorsStats) {
	...
	for _, nic := range ic.Nics {
		...
		timeDiff := ic.IndicatorsStats.UpdateTime.Sub(oldIndicatorsStats.UpdateTime).Seconds()
		if timeDiff == 0 {
			general.Errorf("%s %s two successive IndicatorsStats update time gap less than 1 second", IrqTuningLogPrefix, nic.NicInfo)
			continue
		}
...
func (ic *IrqTuningController) periodicTuning(oldConf *config.IrqTuningConfig) {
	...
	if oldConf != nil && !apiequality.Semantic.DeepEqual(ic.conf.NormalThroughputNics, oldConf.NormalThroughputNics) {
		...
		general.Infof("%s static configured normal throughput nics changed", IrqTuningLogPrefix)
		general.Infof("%s old static configured normal throughput nics:", IrqTuningLogPrefix)
		for _, nic := range oldConf.NormalThroughputNics {
			if nic.NetNSName == "" {
				general.Infof("%s   %s", IrqTuningLogPrefix, nic.NicName)
			} else {
				general.Infof("%s   %s/%s", IrqTuningLogPrefix, nic.NetNSName, nic.NicName)
			}
		}

		general.Infof("%s new static configured normal throughput nics:", IrqTuningLogPrefix)
		for _, nic := range ic.conf.NormalThroughputNics {
			if nic.NetNSName == "" {
				general.Infof("%s   %s", IrqTuningLogPrefix, nic.NicName)
			} else {
	...
	nicAffinitySocketsPolicyChanged := false
	if oldConf != nil && ic.conf.NicAffinitySocketsPolicy != oldConf.NicAffinitySocketsPolicy {
		nicAffinitySocketsPolicyChanged = true
		general.Infof("%s NicAffinitySocketsPolicy changed from %s to %s",
			IrqTuningLogPrefix, oldConf.NicAffinitySocketsPolicy, ic.conf.NicAffinitySocketsPolicy)
	}

	if nicsChanged || staticNormalThroughputNicsChanged || nicAffinitySocketsPolicyChanged {
		if !nicsChanged {
